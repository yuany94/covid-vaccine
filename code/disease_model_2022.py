...
#         self._build_proportions_matrix(poi_cbg_proportions)
        ...
        self.vaccination_rate = np.array(vaccination_rate) * vaccine_efficacy
        ...
        self.NATURAL_IMMUNITY_PERIOD = NATURAL_IMMUNITY_PERIOD
...
#         np.save('tmp.npy', self.POI_CBG_PROPORTIONS)
    ...
    def init_endogenous_variables(self):
        ...
        # different p0
        multiplier = np.zeros(self.N)
        multiplier = 1 - vaccination_rate
        self.P0 = self.P0 * multiplier
        
        self.P0 = np.minimum(self.P0, self.CBG_SIZES)
        assert len(vaccination_rate) == self.N
        
        self.cbg_latent = self.P0
        self.cbg_infection_count = np.zeros((self.num_seeds, self.N))
        self.cbg_infected = np.zeros((self.num_seeds, self.N))
        self.cbg_infected = np.zeros((self.num_seeds, self.N))
        self.cbg_removed = np.zeros((self.num_seeds, self.N))
        self.cbg_removed = np.array([vaccination_rate * self.CBG_SIZES] * self.num_seeds).astype(int)
        ...
        # dynamic, CBG proportions for current dayp_sick_at_t0
...
#             (end_time - start_time, (end_time - start_time)/t))
    ...
    def update_states(self, t):
        ...
        new_susceptible = self.get_new_susceptible()

        if not self.just_compute_r0:
            # normal case.
            self.cbg_infection_count += new_infectious
            ...
            self.cbg_removed = self.cbg_removed + new_removed - new_susceptible
        else:
            ...
            self.cbg_infected = self.cbg_infected + new_infectious - new_removed
            self.cbg_removed = self.cbg_removed + new_removed + self.cbg_new_cases - new_susceptible
    ...
    def get_new_susceptible(self):
        '''
        Individuals leave R at a rate inversely proportional to the removed period.
        '''
        if self.NATURAL_IMMUNITY_PERIOD == 0:
            return 0
        else:
            new_susceptible = np.random.binomial(self.cbg_removed.astype(int), 1 / self.NATURAL_IMMUNITY_PERIOD)
        return new_susceptible
...
# m.init_exogenous_variables(poi_cbg_proportions=poi_cbg_proportions_int_keys, # a dict d[aggregate_col_to_use].values  # an array of dicts; each dict represents CBG distribution for POI
#                            poi_time_counts=poi_time_counts, #poi_time_counts
#                            poi_areas=poi_areas, # likely an array
